

<!DOCTYPE html>
<html>
<head>
	  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
	<title></title>

  <style type="text/css">

    .img
{
background-image:url("https://th.bing.com/th/id/R.3ba9c349900cb094cffc7f2c099b7692?rik=rAolWQnMJAqZrA&riu=http%3a%2f%2fthegctv.com%2fwp-content%2fuploads%2f2019%2f05%2fQuotes-From-Commencement-Speeches.jpg&ehk=zH%2fhUh%2fTnp8xunqmDTgiJhGH8VQt4BMTaUd6JK7miIw%3d&risl=&pid=ImgRaw&r=0");
}

  </style>
</head>
<body>
    
<?php
include('./components/header.php');
?>


  
<div class=" img">

	<h1 id="content">
		About this page:
	</h1>

	<p id="para">
        This page is made to make eazy for the lectures to create question paper <br>
        (step 1) : Login using Login admin button <br>
        (step 2) : 
	</p>
<script type="text/javascript">
   let txt1 = document.getElementById("content");
   let txt2 = document.getElementById("para");
            txt1.style.textAlign = "center";
            txt2.style.textAlign = "center";
</script>

    <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br>



</div>

</body>
</html>